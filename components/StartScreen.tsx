...
interface StartScreenProps {
  ...
  hasSave?: boolean;
  onContinue?: (aiEnabled: boolean, lang: Language) => void;
}
    ...
    <div className="absolute inset-0 flex flex-col items-center justify-center z-50 text-white font-sans p-6 bg-black/30 backdrop-blur-sm transition-all duration-1000">
      <div className="max-w-md w-full bg-slate-900/90 p-8 rounded-2xl border border-slate-700 shadow-2xl backdrop-blur-xl relative overflow-hidden animate-fade-in">
        ...
        <div className="relative z-10">
          ...
          {hasSave && onContinue && (
            <button
              onClick={() => onContinue(aiEnabled, lang)}
              className="w-full py-4 mb-3 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-bold rounded-xl shadow-lg shadow-emerald-900/20 transform transition-all hover:scale-[1.02] active:scale-[0.98] text-lg tracking-wide border border-emerald-500/30"
            >
              {t('continue_game')}
            </button>
          )}
